...
export default function ModifyReviewers() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([jobRes, reviewersRes, assignedRes]) => {
        setJob(jobRes.data);
        setReviewers(reviewersRes.data);
        
        // Pre-select already assigned reviewers
        const assignedReviewerIds = assignedRes.data.map((reviewer) => reviewer.reviewerId);
  ...
  return (
    <Box>
      ...
      <Box display="flex" gap={2}>
        ...
        >
          {submitting ? <CircularProgress size={24} /> : "Update Reviewers"}
        </Button>
        <Button
          variant="outlined"
          size="large"
