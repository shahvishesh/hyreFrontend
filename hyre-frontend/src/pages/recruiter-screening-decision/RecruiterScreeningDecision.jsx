...
export default function RecruiterScreeningDecision() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(
        ([candidateRes, jobRes, reviewRes, skillsRes]) => {
  ...
  const handleDecision = async (decision) => {
    try {
      ...
      await axiosInstance.put("/api/CandidateReview/recruiter-decision", {
        reviewID: review.reviewID,
        decision: decision,
      });

      toast.success(`Candidate ${decision} successfully`);
      navigate(`/dashboard/recruiter-screening/${jobId}`, { replace: true });
    } catch (err) {
      toast.error(
  ...
  return (
    <Box>
      ...
      <Typography variant="h5" mb={3}>
        Screening Review - Make Decision
      </Typography>
          ...
          <Grid container spacing={2}>
            ...
            {review.recruiterDecision && (
              <Grid item xs={12} sm={6}>
                ...
                <Chip label={review.recruiterDecision} size="small" />
              </Grid>
            )}
      ...
      <Box display="flex" gap={2}>
        <Button
          variant="contained"
          color="success"
          disabled={saving || review.recruiterDecision}
          onClick={() => handleDecision("Shortlisted")}
        >
          {saving ? "Saving..." : "Accept for Interview"}
        </Button>
        ...
        {review.recruiterDecision && (
          <Typography variant="body2" color="text.secondary" alignSelf="center">
            Decision already made: {review.recruiterDecision}
          </Typography>
        )}
      </Box>
    </Box>
